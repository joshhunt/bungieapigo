...
export class ApiMethodParameter {
  constructor(public info: ParameterObject | ReferenceObject) {}
  ...
  importTypeName(
    param: ParameterObject | ReferenceObject = this.info
  ): string | null {
    if ("schema" in param) {
  ...
  parameterParseToString(
    info: ParameterObject | ReferenceObject | SchemaObject = this.info
  ) {
  ...
  parameterParseString(
    ...
    if (info.schema && "type" in info.schema && info.schema.type == "array") {
      ...
      return `${this.name()}.map((${ApiDocHelper.getObjectType(
        items
      )} ${this.name()})=>${this.parameterParseString(items)}).toList()`;
    }
