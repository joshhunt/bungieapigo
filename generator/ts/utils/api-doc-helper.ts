...
export class ApiDocHelper {
  ...
  static getSerializeFunction(
    ...
    if (param.additionalProperties && param["x-dictionary-key"]) {
      let value =
        this.getSerializeFunction(param.additionalProperties, "v") || "v";
  ...
  static getImportInfo(
    ...
    if ("$ref" in obj) {
      let ref = this.getRef(refObj.$ref);
      if (ref.type == "object") {
        return new ImportInfo(this.getRefClassName(refObj.$ref), "models");
      }
    }
