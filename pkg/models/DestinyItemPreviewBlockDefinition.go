...
// Items like Sacks or Boxes can have items that it shows in-game when you view details that
// represent the items you can obtain if you use or acquire the item.
...
type DestinyItemPreviewBlockDefinition struct {
	...
	// leverage this for your own custom screen detail views. Note, however, that these are
	// arbitrarily defined by designers: there's no guarantees of a fixed, known number of these - so
	// fall back to something reasonable if you don't recognize it.
	...
	// If this item should show you Artifact information when you preview it, this is the hash
	// identifier of the DestinyArtifactDefinition for the artifact whose data should be shown.
	ArtifactHash int `json:"artifactHash"`

	// If the preview has an associated action (like "Open"), this will be the localized string for
	// that action.
