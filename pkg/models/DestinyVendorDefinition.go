...
// - Kiosks/Collections, which are really just Vendors that don't charge currency (or charge
// some pittance of a currency) and whose gating for purchases revolves more around your
// character's state.
// - Previews for rewards or the contents of sacks. These are implemented as Vendors, where you
// can't actually purchase from them but the items that they have for sale and the categories of
// sale items reflect the rewards or contents of the sack. This is so that the game could reuse the
// existing Vendor display UI for rewards and save a bunch of wheel reinvention.
// - Item Transfer capabilities, like the Vault and Postmaster. Vendors can have "acceptedItem"
// buckets that determine the source and destination buckets for transfers. When you interact
// with such a vendor, these buckets are what gets shown in the UI instead of any items that the
// Vendor would have for sale. Yep, the Vault is a vendor.
// It is pretty much guaranteed that they'll be used for even more features in the future. They have
// come to be seen more as generic categorized containers for items than "vendors" in a
// traditional sense, for better or worse.
// Where possible and time allows, we'll attempt to split those out into their own more digestible
// derived "Definitions": but often time does not allow that, as you can see from the above ways
...
type DestinyVendorDefinition struct {
	...
	// The type of reward progression that this vendor has. Default - The original rank progression
	// from token redemption. Ritual - Progression from ranks in ritual content. For example:
	// Crucible (Shaxx), Gambit (Drifter), and Battlegrounds (War Table).
	VendorProgressionType DestinyVendorProgressionType `json:"vendorProgressionType"`
	...
	// The game UI and BNet often mine the faction definition for additional elements and details to
	// place on the screen, such as the faction's Progression status (aka "Reputation").
	...
	// Again, used for reset/refreshing of inventory. Don't worry too much about it. Unless you want
	// to.
	...
	// reasons why not.
	// This is a list of localized strings with messages for those custom failures. The live BNet data
	// will return a failureIndexes property for items that can't be purchased: using those values to
	// index into this array, you can show the user the appropriate failure message for the item that
	// can't be bought.
	...
	// list of those date ranges. Sadly, we're not able to predict this very frequently, so this will
	// often be useless data.
	UnlockRanges []DateRange `json:"unlockRanges"`

	// The internal identifier for the Vendor. A holdover from the old days of Vendors, but we don't
	// have time to refactor it away.
	...
	// theoretically a Vendor could let you interact with it by performing actions. We'll see what
	// these end up looking like if they ever get used.
	Actions []DestinyVendorActionDefinition `json:"actions"`

	// These are the headers for sections of items that the vendor is selling. When you see items
	// organized by category in the header, it is these categories that it is showing.
	// Well, technically not *exactly* these. On BNet, it doesn't make sense to have categories be
	// "paged" as we do in Destiny, so we run some heuristics to attempt to aggregate pages of
	// categories together.
	// These are the categories post-concatenation, if the vendor had concatenation applied. If you
	// want the pre-aggregated category data, use originalCategories.
	Categories []DestinyVendorCategoryEntryDefinition `json:"categories"`

	// See the categories property for a description of categories and why originalCategories
	// exists.
	...
	// The "categories" structure is for validation of the contained items, and can be categorized
	// entirely separately from "Display Categories", there need be and often will be no meaningful
	// relationship between the two.
	DisplayCategories []DestinyDisplayCategoryDefinition `json:"displayCategories"`

	// In addition to selling items, vendors can have "interactions": UI where you "talk" with the
	// vendor and they offer you a reward, some item, or merely acknowledge via dialog that you did
	// something cool.
	...
	// is the list of those items that the vendor can sell. From this list, only a subset will be
	// available from the vendor at any given time, selected randomly and reset on the vendor's
	// refresh interval.
	// Note that a vendor can sell the same item multiple ways: for instance, nothing stops a vendor
	// from selling you some specific weapon but using two different currencies, or the same weapon at
	// multiple "item levels".
	ItemList []DestinyVendorItemDefinition `json:"itemList"`

	// BNet doesn't use this data yet, but it appears to be an optional list of flavor text about
	// services that the Vendor can provide.
	...
	// effort to reduce this workload, only Vendors with this flag set will be returned on Vendor
	// requests. This allows us to filter out Vendors that don't dynamic data that's particularly
	// useful: things like "Preview/Sack" vendors, for example, that you can usually suss out the
	// details for using just the definitions themselves.
	ReturnWithVendorRequest bool `json:"returnWithVendorRequest"`

	// A vendor can be at different places in the world depending on the game/character/account
	// state. This is the list of possible locations for the vendor, along with conditions we use to
	...
	// either location or function/purpose. It's used for our our Companion Vendor UI. Only one of
	// these can be active for a Vendor at a time.
	Groups []DestinyVendorGroupReference `json:"groups"`

	// Some items don't make sense to return in the API, for example because they represent an action to
	// be performed rather than an item being sold. I'd rather we not do this, but at least in the short
	// term this is a workable workaround.
