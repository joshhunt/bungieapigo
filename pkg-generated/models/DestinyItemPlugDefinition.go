...
// This gives information about when it can be inserted, what the plug's category is (and thus
// whether it is compatible with a socket... see DestinySocketTypeDefinition for information
// about Plug Categories and socket compatibility), whether it is enabled and other Plug info.
type DestinyItemPlugDefinition struct {

	// The rules around when this plug can be inserted into a socket, aside from the socket's
	// individual restrictions.
	// The live data DestinyItemPlugComponent.insertFailIndexes will be an index into this array,
	// so you can pull out the failure strings appropriate for the user.
	...
	// DestinySocketTypeDefinition.plugWhitelist to determine whether this plug can be inserted
	// into the socket.
	...
	// If you successfully socket the item, this will determine whether or not you get "refunded" on
	// the plug.
	...
	// may require materials. This is the hash identifier for the
	...
	// The live data DestinyItemPlugComponent.enableFailIndexes will be an index into this array,
	// so you can pull out the failure strings appropriate for the user.
	EnabledRules []DestinyPlugRuleDefinition `json:"enabledRules"`

	// Plugs can have arbitrary, UI-defined identifiers that the UI designers use to determine the
	// style applied to plugs. Unfortunately, we have neither a definitive list of these labels nor
	// advance warning of when new labels might be applied or how that relates to how they get rendered.
	// If you want to, you can refer to known labels to change your own styles: but know that new ones can
	// be created arbitrarily, and we have no way of associating the labels with any specific UI style
	...
	PlugStyle PlugUiStyles `json:"plugStyle"`

	// Indicates the rules about when this plug can be used. See the PlugAvailabilityMode
	...
	AlternatePlugStyle PlugUiStyles `json:"alternatePlugStyle"`

	// If TRUE, this plug is used for UI display purposes only, and doesn't have any interesting
	// effects of its own.
	IsDummyPlug bool `json:"isDummyPlug"`

	// Do you ever get the feeling that a system has become so overburdened by edge cases that it
	// probably should have become some other system entirely? So do I!
	// In totally unrelated news, Plugs can now override properties of their parent items. This is
	// some of the relevant definition data for those overrides.
	...
	// for example, is implemented in a similar way to Masterworks, where visually it's a single area
	// of the UI being clicked on to "Upgrade" to higher energy levels, but it's actually socketing new
	// plugs.
