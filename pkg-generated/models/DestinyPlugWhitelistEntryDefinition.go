...
type DestinyPlugWhitelistEntryDefinition struct {
    ...
    // The hash identifier of the Plug Category to compare against the plug item's
    // plug.plugCategoryHash.
    // Note that this does NOT relate to any Definition in itself, it is only used for comparison
    // purposes.
    ...
    // Which ones you should actually show are determined by the plug being inserted into the socket,
    // and the socket’s type.
    ...
    // property.If it does, that means we know what it will roll if you try to insert this plug into this
    // socket.
