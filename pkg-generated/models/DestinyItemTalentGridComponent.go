...
type DestinyItemTalentGridComponent struct {
    ...
    // You can use this hash to lookup the DestinyTalentGridDefinition attached to this item, which
    // will be crucial for understanding the node values on the item.
    ...
    // may have multiple "steps" which indicate the actual bonuses and visual representation of that
    // node.
    Nodes []DestinyTalentNode `json:"nodes"`


    // Indicates whether the talent grid on this item is completed, and thus whether it should have a
    // gold border around it.
    // Only will be true if the item actually *has* a talent grid, and only then if it is completed (i.e.
    // every exclusive set has an activated node, and every non-exclusive set node has been
    // activated)
    IsGridComplete bool `json:"isGridComplete"`


    // If the item has a progression, it will be detailed here. A progression means that the item can
    // gain experience. Thresholds of experience are what determines whether and when a talent node
    // can be activated.
