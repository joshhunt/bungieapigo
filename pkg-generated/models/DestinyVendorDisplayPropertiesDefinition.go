...
type DestinyVendorDisplayPropertiesDefinition struct {
    ...
    // according to the game's content. It may be more lame and/or have less razzle-dazzle. But who am I
    // to tell you which icon to use.
    OriginalIcon string `json:"originalIcon"`


    // Vendors, in addition to expected display property data, may also show some "common
    // requirements" as statically defined definition data. This might be when a vendor accepts a
    // single type of currency, or when the currency is unique to the vendor and the designers wanted to
    // show that currency when you interact with the vendor.
    ...
    // This is apparently the "Watermark". I am not certain offhand where this is actually used in the
    // Game UI, but some people may find it useful.
