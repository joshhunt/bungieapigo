...
// *highly* variable what kind of data we'll actually be able to return: at times we may have no
// other relationships to entities at all.
...
type DestinyChecklistEntryDefinition struct {
    ...
    // The identifier for this Checklist entry. Guaranteed unique only within this Checklist
    // Definition, and not globally/for all checklists.
    Hash int `json:"hash"`


    // Even if no other associations exist, we will give you *something* for display properties. In
    // cases where we have no associated entities, it may be as simple as a numerical identifier.
    ...
    // You can use this property if it is populated to look up the DestinyLocationDefinition's
    // associated .locationReleases[].activityBubbleName property.
