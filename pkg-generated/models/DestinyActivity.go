...
// Represents the "Live" data that we can obtain about a Character's status with a specific
// Activity. This will tell you whether the character can participate in the activity, as well as
// some other basic mutable information.
// Meant to be combined with static DestinyActivityDefinition data for a full picture of the
// Activity.
type DestinyActivity struct {
	...
	// completed it. Unfortunately, we can't necessarily know this for all activities. As such, this
	// should probably only be used if you already know in advance which specific activities you wish
	// to check.
	...
	DifficultyTier DestinyActivityDifficultyTier `json:"difficultyTier"`
	...
	// referred to: those are all *possible* modifiers for the activity, not the active ones. Use only
	// the active ones to match what's really live.
	...
	// activity (not guaranteed to be unique between or across all activities, though should be
	// unique for every *variant* of a given *conceptual* activity: for instance, the original D2
	// Raid has many variant DestinyActivityDefinitions. While other activities could
	// potentially have the same option hashes, for any given D2 base Raid variant the hash will be
	// unique).
	// As a concrete example of this data, the hashes you get for Raids will correspond to the currently
	// active "Challenge Mode".
	// We don't have any human readable information for these, but saavy 3rd party app users could
	// manually associate the key (a hash identifier for the "option" that is enabled/disabled) and
	// the value (whether it's enabled or disabled presently)
	// On our side, we don't necessarily even know what these are used for (the game designers know, but
	// we don't), and we have no human readable data for them. In order to use them, you will have to do
	// some experimentation.
	BooleanActivityOptions map[int]bool `json:"booleanActivityOptions"`
