...
// Sometimes, we have large sets of reusable plugs that are defined identically and thus can (and
// in some cases, are so large that they *must*) be shared across the places where they are used.
...
// relationship between these reusable plug sets and the sockets that leverage them (for
// starters, Emotes).
...
// information. We will make a best effort to guess what ought to be populated there where
// possible, but it will be invalid for many/most plug sets.
type DestinyPlugSetDefinition struct {
	...
	// This is a list of pre-determined plugs that can be plugged into this socket, without the
	...
	// Mostly for our debugging or reporting bugs, BNet is making "fake" plug sets in a desperate
	// effort to reduce socket sizes.
