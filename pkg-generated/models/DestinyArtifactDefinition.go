...
// we've been told that they will be replaced between seasons. But this definition is built both to
// minimize the amount of lookups for related data that have to occur, and is built in hope that, if
// this plan changes, we will be able to accommodate it more easily.
type DestinyArtifactDefinition struct {

	// Any basic display info we know about the Artifact. Currently sourced from a related inventory
	// item, but the source of this data is subject to change.
	DisplayProperties DestinyDisplayPropertiesDefinition `json:"displayProperties"`

	// Any Geometry/3D info we know about the Artifact. Currently sourced from a related inventory
	// item's gearset information, but the source of this data is subject to change.
