...
// Plugs to be inserted, what Categories of Plugs can be inserted, and whether the socket is even
// visible at all given the current game/character/account state.
...
type DestinySocketTypeDefinition struct {
	...
	// DestinyInventoryItemDefinition.plug.plugCategoryHash, which indicates the plug item's
	// category.
	// If the plug's category matches any whitelisted plug, or if the whitelist is empty, it is allowed
	// to be inserted.
	...
	// type are not visible. Unfortunately, the truth of visibility is much, much more complex. Best
	// to rely on the live data for whether the socket is visible and enabled.
