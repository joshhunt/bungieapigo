...
// An individual Activity Mode represents a collection of activities that are played in a certain
// way. For example, Nightfall Strikes are part of a "Nightfall" activity mode, and any
...
// "AllPvP", you will get any PVP activities that the user has played, regardless of what specific
// PVP mode was being played.
type DestinyActivityModeDefinition struct {
	...
	ModeType DestinyActivityModeType `json:"modeType"`

	// The type of play being performed in broad terms (PVP, PVE)
	ActivityModeCategory DestinyActivityModeCategory `json:"activityModeCategory"`

	// If True, this mode has oppositional teams fighting against each other rather than
	// "Free-For-All" or Co-operative modes of play.
	...
	// itself. This includes modes that group Features/Events rather than Gameplay, such as Trials
	// of The Nine: Trials of the Nine being an Event that is interesting to see aggregate data for, but
	// when you play the activities within Trials of the Nine they are more specific activity modes
	// such as Clash.
	IsAggregateMode bool `json:"isAggregateMode"`

	// The hash identifiers of the DestinyActivityModeDefinitions that represent all of the
	// "parent" modes for this mode. For instance, the Nightfall Mode is also a member of AllStrikes
	// and AllPvE.
	ParentHashes []int `json:"parentHashes"`

	// A Friendly identifier you can use for referring to this Activity Mode. We really only used this
	// in our URLs, so... you know, take that for whatever it's worth.
	FriendlyName string `json:"friendlyName"`

	// If this exists, the mode has specific Activities (referred to by the Key) that should instead
	// map to other Activity Modes when they are played. This was useful in D1 for Private Matches,
	// where we wanted to have Private Matches as an activity mode while still referring to the
	// specific mode being played.
	ActivityModeMappings map[int]DestinyActivityModeType `json:"activityModeMappings"`

	// If FALSE, we want to ignore this type when we're showing activity modes in BNet UI. It will still
	// be returned in case 3rd parties want to use it for any purpose.
