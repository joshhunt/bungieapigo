...
// Talent Grids on items have Nodes. These nodes have positions in the talent grid's UI, and
// contain "Steps" (DestinyTalentNodeStepDefinition), one of whom will be the "Current" step.
// The Current Step determines the visual properties of the node, as well as what the node grants
// when it is activated.
// See DestinyTalentGridDefinition for a more complete overview of how Talent Grids work, and
// how they are used in Destiny 2 (and how they were used in Destiny 1).
type DestinyTalentNodeDefinition struct {

	// The index into the DestinyTalentGridDefinition's "nodes" property where this node is
	// located. Used to uniquely identify the node within the Talent Grid. Note that this is content
	// version dependent: make sure you have the latest version of content before trying to use these
	// properties.
	NodeIndex int `json:"nodeIndex"`

	// The hash identifier for the node, which unfortunately is also content version dependent but
	// can be (and ideally, should be) used instead of the nodeIndex to uniquely identify the node.
	...
	// Indexes into the DestinyTalentGridDefinition.nodes property for any nodes that must be
	// activated before this one is allowed to be activated.
	...
	// mostly because Exclusive Sets are *almost* a superset of this concept, but the potential for it
	// to be used still exists in theory.
	// If this is ever used, this will be the index into the DestinyTalentGridDefinition.nodes
	// property for the node that is the binary pair match to this node. Activating one deactivates the
	// other.
	BinaryPairNodeIndex int `json:"binaryPairNodeIndex"`

	// If true, this node will automatically unlock when the Talent Grid's level reaches the required
	// level of the current step of this node.
	AutoUnlocks bool `json:"autoUnlocks"`

	// At one point, Nodes were going to be able to be activated multiple times, changing the current
	// step and potentially piling on multiple effects from the previously activated steps. This
	// property would indicate if the last step could be activated multiple times.
	// This is not currently used, but it isn't out of the question that this could end up being used
	// again in a theoretical future.
	...
	// "re-roll" the current step of the node (and thus change the properties of your item). This was to
	// be the activation requirement for performing that re-roll.
	...
	// If this is true, the node can be "re-rolled" to acquire a different random current step. This is
	// not used, but still exists for a theoretical future of talent grids.
	IsRandomRepurchasable bool `json:"isRandomRepurchasable"`

	// At this point, "steps" have been obfuscated into conceptual entities, aggregating the
	// underlying notions of "properties" and "true steps".
	...
	// A string identifier for a custom visual layout to apply to this talent node. Unfortunately, we
	// do not have any data for rendering these custom layouts. It will be up to you to interpret these
	...
	// See DestinyTalentExclusiveGroup for more information on the details. This is an identifier
	// for this node's group, if it is part of one.
	...
	// Comes from the talent grid node style: this identifier should be used to determine how to render
	// the node in the UI.
