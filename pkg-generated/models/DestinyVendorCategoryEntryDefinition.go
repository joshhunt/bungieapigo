...
type DestinyVendorCategoryEntryDefinition struct {
	...
	// Used in sorting items in vendors... but there's a lot more to it. Just go with the order provided
	// in the itemIndexes property on the DestinyVendorCategoryComponent instead, it should be
	// more reliable than trying to recalculate it yourself.
	...
	// If you don't have the currency required to buy items from this category, should the items be
	// hidden?
	...
	// The localized string for making purchases from this category, if it is different from the
	// vendor's string for purchasing.
	...
	// If this category has an overlay prompt that should appear, this contains the details of that
	// prompt.
	...
	// Sometimes a category isn't actually used to sell items, but rather to preview them. This
	// implies different UI (and manual placement of the category in the UI) in the game, and special
	// treatment.
